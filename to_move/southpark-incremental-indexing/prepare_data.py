...
def read_data(data_fn, output_fn_1, output_fn_2):
    _min_sent_len = 3
    _max_sent_len = 64
    punct_chars = [
        '!',
    ...
    doc_list1 = doc_list[: len(doc_list) // 2]
    doc_list2 = doc_list[len(doc_list) // 2 :]
    files = [output_fn_1, output_fn_2]
    for output_f, doc_list_subset in zip(files, [doc_list1, doc_list2]):
        with open(output_f, 'w') as f:
            f.write('\n'.join(doc_list_subset))
    print(f'wrote data to {files}')


if __name__ == '__main__':
    data_dir = sys.argv[1]
    read_data(
        os.path.join(data_dir, 'All-seasons.csv'),
        os.path.join(data_dir, 'character-lines-1.csv'),
        os.path.join(data_dir, 'character-lines-2.csv'),
    )
