...
export $(grep -v '^#' tests/hyperparameter/.env | xargs -d '\n')

docker-compose -f tests/hyperparameter/docker-compose.yml --project-directory . up  --build -d

sleep 5

python tests/hyperparameter/optimize.py
...
#     -F "yamlspec=@tests/hyperparameter/flow-query.yml"\
#     | jq -r .flow_id)
...
# docker-compose -f tests/hyperparameter/docker-compose.yml --project-directory . down
