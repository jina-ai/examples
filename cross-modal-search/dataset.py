...
def input_index_data(num_docs=None, batch_size=8, dataset_type='f30k'):
    ...
    for i, (images, captions) in enumerate(data_loader):
        for image, caption in zip(images, captions):
            ...
            # document_img.tags["dimensions"] = document_img.blob.shape[0:2]

            width, height = (document_img.blob.shape[1], document_img.blob.shape[0])
            document_img.convert_image_blob_to_uri(width=width, height=height)
            document_img.tags["uri"] = document_img.uri
            document_img.buffer = image
