...
def config():
    os.environ["JINA_DATA_FILE"] = os.environ.get(
        "JINA_DATA_FILE", "data/people_wiki.csv"
    )
...
def print_topk(resp, query_text):
    for d in resp.search.docs:
        print(f"Ta-Dah🔮, here are what we found for: {query_text}")
        for idx, match in enumerate(d.matches):
            ...
            uri = match.tags['uri']
            print(f"> {idx:>2d}({score:.2f}). {name}: {text}")
